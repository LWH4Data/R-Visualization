...
pm <- p + geom_line(aes(y = mean(libido))) +
  geom_segment(aes(x = 1, xend = 5, y = 2.2, yend = 2.2), color='yellow', size = 1.2) +
  geom_segment(aes(x = 6, xend = 10, y = 3.2, yend = 3.2), color = 'green', size = 1.2) +
  geom_segment(aes(x = 11, xend = 15, y = 5, yend = 5), color = 'blue', size = 1.2)
pm

#legend manipulate
pm
pm + scale_shape_discrete(name = "복용", 
                          breaks = c("W", "L", "H"),
                          labels = c("위약", "저용량", "고용량")) +
  scale_color_manual(values = c("yellow", "green", "blue"), name = "복용", 
                     breaks = c("W", "L", "H"),
                     labels = c("위약", "저용량", "고용량"))